...
export const defaultBundles: BundleOption[] = createBundles(45, 'mist');
export const showerFilterBundles: BundleOption[] = createBundles(118, 'showerhead');
        ...
        {bundles.map((bundle, index) => {
          ...
          return (
            <motion.button
              ...
              className={cn(
                "w-full flex items-center justify-between p-2.5 md:p-4 rounded-xl border-2 transition-all duration-300 text-left relative overflow-hidden",
                isSelected
            ...
            >
              {/* Quantity Badge */}
              <div className={cn(
                "absolute -top-1 -left-1 w-9 h-9 md:w-12 md:h-12 flex items-center justify-center rounded-br-2xl font-bold text-xs md:text-base transition-all duration-300",
                isSelected 
              ...
              <div className="flex items-center gap-2 md:gap-3 ml-9 md:ml-12">
                {/* Radio indicator */}
                <div
                  className={cn(
                    "w-4 h-4 md:w-6 md:h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all duration-300",
                ...
                >
                  {isSelected && (
                    <motion.div 
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-accent" 
                    />
                  )}
                </div>
                ...
                <div>
                  <div className="flex items-center gap-1 md:gap-2 flex-wrap">
                    ...
                    {bundle.isPopular && (
                      <motion.span 
                        ...
                        className="flex items-center gap-0.5 text-[9px] md:text-xs font-semibold text-amber-500 bg-amber-500/10 px-1.5 md:px-2 py-0.5 rounded-full"
                      >
                    ...
                    {bundle.isBestValue && (
                      <motion.span 
                        initial={{ scale: 0.8, opacity: 0 }}
                        animate={{ scale: 1, opacity: 1 }}
                        className="flex items-center gap-0.5 text-[9px] md:text-xs font-semibold text-accent bg-accent/10 px-1.5 md:px-2 py-0.5 rounded-full"
                      >
                  ...
                  <div className="flex items-center gap-1.5 md:gap-2 mt-0.5">
                    ...
                    {bundle.quantity > 1 && (
                      <span className="text-[9px] md:text-xs text-accent font-medium">
              ...
              <div className="text-right">
                ...
                {bundle.badge && (
                  <motion.span 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="inline-block mt-0.5 md:mt-1 text-[9px] md:text-xs font-bold text-white bg-gradient-to-r from-red-500 to-red-600 px-1.5 md:px-2 py-0.5 rounded-full shadow-sm"
                  >
                    {bundle.badge}
                  </motion.span>
                )}
